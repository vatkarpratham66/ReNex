...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const donors = await Donor.find().populate("user_id", "name email");
 ...
 * Donor sees which NGO accepted donations.
 */
router.get("/my/details", auth, requireRole("donor"), async (req, res) => {
  try {
    ...
    const accepts = await Accept.find({ donor_id: donor._id })
      .populate("ngo_id", "ngo_name registration_no")
      .select("-__v");

    res.json({ donations: donor.donations, accepts });
  } catch (e) {
    res.status(500).json({ msg: "Server error" });
  }
});
...
router.delete("/:donationId", auth, requireRole("donor"), async (req, res) => {
  try {
    ...
    if (donation.status !== "available") {
      return res.status(400).json({ msg: "Only available donations can be deleted" });
    }
